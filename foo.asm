...
L_constants:
	...
	; L_constants + 1864:
	db T_string	; "dummy_value"
	dq 11
	db 0x64, 0x75, 0x6D, 0x6D, 0x79, 0x5F, 0x76, 0x61
	db 0x6C, 0x75, 0x65
	; L_constants + 1884:
	db T_interned_symbol	; dummy_value
	dq L_constants + 1864
	; L_constants + 1893:
	db T_string	; "apply"
	dq 5
...
.L_lambda_opt_env_end_0f51:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2df1:	; copy params
	cmp rsi, 0
	je .L_lambda_opt_params_end_2df1
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2df1
.L_lambda_opt_params_end_2df1:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f51:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f51 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f51 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f51
.L_lambda_opt_arity_check_exact_0f51:
	;;;;goal of this label: lower the params location by 1 , so that we could have room for sob_nil

	mov r9, 3 + 0
	sub rsp, 8
	mov qword r8, rsp ; r8 -> starting address of where to put parameters.
.L_lambda_opt_params_loop_2df2:
	;;;;push the arguments in rcx, one after another

	mov qword rcx, [r8 + 8]
	mov qword [r8], rcx
	add r8, 8
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2df2
.L_lambda_opt_params_end_2df2:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
	jmp .L_lambda_opt_stack_adjusted_0f51
.L_lambda_opt_arity_check_more_0f51:
	;;;;the point of this label is to find how many optional parameters are there.

	mov rax, sob_nil
	mov r13, [rsp + 16]
	mov r10, [rsp + 16] ; we save in two different registers because one is going to be changed later.
	lea rbx, [rsp + 16 + 8 * r10] ; calculate the address and put in rbx
	sub r10, 0
.L_lambda_opt_stack_shrink_loop_0f51:
	;;;;rax will always contain the parameter. initial value : NIL. (we create a list of optional parameters)

	mov rdx, [rbx] ; move the VALUE of the parameter to rdx.
	mov rcx, rax
	mov rdi, 17 ; allocate size
	call malloc ; address is returned to rax
	mov byte [rax], T_pair
	mov SOB_PAIR_CAR(rax), rdx ; list is constructed by pairs.
	mov SOB_PAIR_CDR(rax), rcx
	sub rbx, 8 ; get next param
	dec r10 ; decrease params_length
	cmp r10, 0
	jg .L_lambda_opt_stack_shrink_loop_0f51
.L_lambda_opt_stack_shrink_loop_exit_0f51:
	mov [rsp + 16 + 8 * 1], rax
	mov r10, 1
	mov [rsp + 16], r10 ; new params length
	mov r14, rsp
	add r14, 16 + 8 * 1 ; jump to the top of the parameters
	sub r13, [rsp + 16] ; r13 will now hold the number of opt parameters
	mov r10, r13
	shl r10, 3 ; make it accessible by multiples of 8
	mov rbx, r10
	add r10, r14 ; add the stack pointer to the number of params
	mov r8, [rsp + 16] ; number of params
	add r8, 3 ; add 3 because of env,ret and list
.L_lambda_opt_params_loop_2df3:
	;;;;add the params one by one
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2df3
.L_lambda_opt_params_end_2df3:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f52:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2df4:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2df4
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2df4
.L_lambda_opt_params_end_2df4:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f52:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f52 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f52 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f52
...
.L_lambda_opt_params_loop_2df5:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2df5
.L_lambda_opt_params_end_2df5:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_stack_shrink_loop_exit_0f52:
	...
	mov [rsp + 16], r10 ; new params length
	mov r14, rsp
	add r14, 16 + 8 * 2 ; jump to the top of the parameters
	sub r13, [rsp + 16] ; r13 will now hold the number of opt parameters
	mov r10, r13
	shl r10, 3 ; make it accessible by multiples of 8
	mov rbx, r10
...
.L_lambda_opt_params_loop_2df6:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2df6
.L_lambda_opt_params_end_2df6:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f53:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2df7:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2df7
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2df7
.L_lambda_opt_params_end_2df7:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f53:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f53 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f53 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f53
...
.L_lambda_opt_params_loop_2df8:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2df8
.L_lambda_opt_params_end_2df8:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2df9:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2df9
.L_lambda_opt_params_end_2df9:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f54:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2dfa:	; copy params
	cmp rsi, 0
	je .L_lambda_opt_params_end_2dfa
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2dfa
.L_lambda_opt_params_end_2dfa:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f54:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f54 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f54 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f54
...
.L_lambda_opt_params_loop_2dfb:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2dfb
.L_lambda_opt_params_end_2dfb:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2dfc:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2dfc
.L_lambda_opt_params_end_2dfc:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f55:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2dfd:	; copy params
	cmp rsi, 0
	je .L_lambda_opt_params_end_2dfd
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2dfd
.L_lambda_opt_params_end_2dfd:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f55:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f55 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f55 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f55
...
.L_lambda_opt_params_loop_2dfe:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2dfe
.L_lambda_opt_params_end_2dfe:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2dff:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2dff
.L_lambda_opt_params_end_2dff:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f56:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e00:	; copy params
	cmp rsi, 2
	je .L_lambda_opt_params_end_2e00
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e00
.L_lambda_opt_params_end_2e00:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f56:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f56 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f56 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f56
...
.L_lambda_opt_params_loop_2e01:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e01
.L_lambda_opt_params_end_2e01:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e02:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e02
.L_lambda_opt_params_end_2e02:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f57:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e03:	; copy params
	cmp rsi, 2
	je .L_lambda_opt_params_end_2e03
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e03
.L_lambda_opt_params_end_2e03:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f57:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f57 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f57 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f57
...
.L_lambda_opt_params_loop_2e04:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e04
.L_lambda_opt_params_end_2e04:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e05:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e05
.L_lambda_opt_params_end_2e05:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f58:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e06:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e06
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e06
.L_lambda_opt_params_end_2e06:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f58:
	mov r10, qword [rsp + 16]
	cmp r10, 2
	je .L_lambda_opt_arity_check_exact_0f58 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f58 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f58
...
.L_lambda_opt_params_loop_2e07:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e07
.L_lambda_opt_params_end_2e07:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_stack_shrink_loop_exit_0f58:
	...
	mov [rsp + 16], r10 ; new params length
	mov r14, rsp
	add r14, 16 + 8 * 3 ; jump to the top of the parameters
	sub r13, [rsp + 16] ; r13 will now hold the number of opt parameters
	mov r10, r13
	shl r10, 3 ; make it accessible by multiples of 8
	mov rbx, r10
...
.L_lambda_opt_params_loop_2e08:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e08
.L_lambda_opt_params_end_2e08:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f59:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e09:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e09
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e09
.L_lambda_opt_params_end_2e09:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f59:
	mov r10, qword [rsp + 16]
	cmp r10, 2
	je .L_lambda_opt_arity_check_exact_0f59 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f59 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f59
...
.L_lambda_opt_params_loop_2e0a:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e0a
.L_lambda_opt_params_end_2e0a:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e0b:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e0b
.L_lambda_opt_params_end_2e0b:
	add rsp, rbx
...
.L_lambda_simple_arity_check_ok_6d1f:
	enter 0, 0
	; preparing a tail-call
	mov rax, L_constants + 2078
	push rax
	mov rax, L_constants + 2069
	push rax
	push 2	; arg count
...
.L_lambda_opt_env_end_0f5a:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e0c:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e0c
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e0c
.L_lambda_opt_params_end_2e0c:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5a:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f5a ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5a ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5a
...
.L_lambda_opt_params_loop_2e0d:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e0d
.L_lambda_opt_params_end_2e0d:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e0e:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e0e
.L_lambda_opt_params_end_2e0e:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f5b:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e0f:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e0f
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e0f
.L_lambda_opt_params_end_2e0f:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5b:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f5b ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5b ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5b
...
.L_lambda_opt_params_loop_2e10:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e10
.L_lambda_opt_params_end_2e10:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e11:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e11
.L_lambda_opt_params_end_2e11:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f5c:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e12:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e12
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e12
.L_lambda_opt_params_end_2e12:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5c:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f5c ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5c ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5c
...
.L_lambda_opt_params_loop_2e13:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e13
.L_lambda_opt_params_end_2e13:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e14:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e14
.L_lambda_opt_params_end_2e14:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f5d:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e15:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e15
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e15
.L_lambda_opt_params_end_2e15:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5d:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f5d ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5d ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5d
...
.L_lambda_opt_params_loop_2e16:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e16
.L_lambda_opt_params_end_2e16:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e17:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e17
.L_lambda_opt_params_end_2e17:
	add rsp, rbx
...
.L_lambda_simple_arity_check_ok_6d32:
	enter 0, 0
	; preparing a tail-call
	mov rax, L_constants + 2299
	push rax
	mov rax, L_constants + 2290
	push rax
	push 2	; arg count
	mov rax, qword [free_var_42]	; free var error
	cmp byte [rax], T_undefined
...
.L_lambda_opt_env_end_0f5e:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e18:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e18
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e18
.L_lambda_opt_params_end_2e18:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5e:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f5e ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5e ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5e
...
.L_lambda_opt_params_loop_2e19:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e19
.L_lambda_opt_params_end_2e19:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e1a:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e1a
.L_lambda_opt_params_end_2e1a:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f5f:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e1b:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e1b
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e1b
.L_lambda_opt_params_end_2e1b:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f5f:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f5f ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f5f ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f5f
...
.L_lambda_opt_params_loop_2e1c:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e1c
.L_lambda_opt_params_end_2e1c:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e1d:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e1d
.L_lambda_opt_params_end_2e1d:
	add rsp, rbx
...
.L_if_else_7710:
	; preparing a tail-call
	mov rax, L_constants + 2368
	push rax
	mov rax, L_constants + 2359
	push rax
	push 2	; arg count
	mov rax, qword [free_var_42]	; free var error
	cmp byte [rax], T_undefined
...
.L_lambda_opt_env_end_0f60:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e1e:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e1e
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e1e
.L_lambda_opt_params_end_2e1e:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f60:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f60 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f60 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f60
...
.L_lambda_opt_params_loop_2e1f:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e1f
.L_lambda_opt_params_end_2e1f:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e20:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e20
.L_lambda_opt_params_end_2e20:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f61:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e21:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e21
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e21
.L_lambda_opt_params_end_2e21:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f61:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f61 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f61 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f61
...
.L_lambda_opt_params_loop_2e22:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e22
.L_lambda_opt_params_end_2e22:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e23:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e23
.L_lambda_opt_params_end_2e23:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f62:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e24:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e24
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e24
.L_lambda_opt_params_end_2e24:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f62:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f62 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f62 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f62
...
.L_lambda_opt_params_loop_2e25:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e25
.L_lambda_opt_params_end_2e25:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e26:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e26
.L_lambda_opt_params_end_2e26:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f63:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e27:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e27
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e27
.L_lambda_opt_params_end_2e27:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f63:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f63 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f63 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f63
...
.L_lambda_opt_params_loop_2e28:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e28
.L_lambda_opt_params_end_2e28:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e29:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e29
.L_lambda_opt_params_end_2e29:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f64:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e2a:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e2a
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e2a
.L_lambda_opt_params_end_2e2a:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f64:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f64 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f64 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f64
...
.L_lambda_opt_params_loop_2e2b:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e2b
.L_lambda_opt_params_end_2e2b:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e2c:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e2c
.L_lambda_opt_params_end_2e2c:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f65:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e2d:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e2d
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e2d
.L_lambda_opt_params_end_2e2d:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f65:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f65 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f65 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f65
...
.L_lambda_opt_params_loop_2e2e:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e2e
.L_lambda_opt_params_end_2e2e:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e2f:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e2f
.L_lambda_opt_params_end_2e2f:
	add rsp, rbx
...
.L_if_else_7724:
	; preparing a non-tail-call
	mov rax, L_constants + 2942
	push rax
	mov rax, L_constants + 2933
	push rax
	push 2	; arg count
	mov rax, qword [free_var_42]	; free var error
	cmp byte [rax], T_undefined
...
.L_lambda_opt_env_end_0f66:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e30:	; copy params
	cmp rsi, 1
	je .L_lambda_opt_params_end_2e30
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e30
.L_lambda_opt_params_end_2e30:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f66:
	mov r10, qword [rsp + 16]
	cmp r10, 1
	je .L_lambda_opt_arity_check_exact_0f66 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f66 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f66
...
.L_lambda_opt_params_loop_2e31:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e31
.L_lambda_opt_params_end_2e31:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e32:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e32
.L_lambda_opt_params_end_2e32:
	add rsp, rbx
...
.L_if_else_7727:
	; preparing a non-tail-call
	mov rax, L_constants + 3003
	push rax
	mov rax, L_constants + 2994
	push rax
	push 2	; arg count
	mov rax, qword [free_var_42]	; free var error
	cmp byte [rax], T_undefined
...
.L_lambda_opt_env_end_0f67:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e33:	; copy params
	cmp rsi, 0
	je .L_lambda_opt_params_end_2e33
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e33
.L_lambda_opt_params_end_2e33:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f67:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f67 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f67 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f67
...
.L_lambda_opt_params_loop_2e34:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e34
.L_lambda_opt_params_end_2e34:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e35:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e35
.L_lambda_opt_params_end_2e35:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f68:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e36:	; copy params
	cmp rsi, 2
	je .L_lambda_opt_params_end_2e36
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e36
.L_lambda_opt_params_end_2e36:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f68:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f68 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f68 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f68
...
.L_lambda_opt_params_loop_2e37:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e37
.L_lambda_opt_params_end_2e37:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e38:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e38
.L_lambda_opt_params_end_2e38:
	add rsp, rbx
...
.L_lambda_opt_env_end_0f69:
	pop rbx
	mov rsi, 0
.L_lambda_opt_params_loop_2e39:	; copy params
	cmp rsi, 2
	je .L_lambda_opt_params_end_2e39
	mov rdx, qword [rbp + 8 * rsi + 8 * 4]
	mov qword [rbx + 8 * rsi], rdx
	inc rsi
	jmp .L_lambda_opt_params_loop_2e39
.L_lambda_opt_params_end_2e39:
	mov qword [rax], rbx	; ext_env[0] <-- new_rib 
	mov rbx, rax
	pop rax
	mov byte [rax], T_closure
...
.L_lambda_opt_code_0f69:
	mov r10, qword [rsp + 16]
	cmp r10, 0
	je .L_lambda_opt_arity_check_exact_0f69 ; if we have no opt parameters
	jg .L_lambda_opt_arity_check_more_0f69 ; if we have at least one opt parameter
	jl L_error_incorrect_arity_opt
	jmp .L_lambda_opt_arity_check_exact_0f69
...
.L_lambda_opt_params_loop_2e3a:
	...
	dec r9 ; r9 holds params_length
	cmp r9, 0
	jg .L_lambda_opt_params_loop_2e3a
.L_lambda_opt_params_end_2e3a:
	inc r10 ; r10 holds argument count
	mov qword [rsp + 16], r10
	add r10, 2 ; get some space for env the list.
	mov qword [rsp + 8 * r10], sob_nil
...
.L_lambda_opt_params_loop_2e3b:
	...
	sub r14, 8
	cmp r8, 0
	jg .L_lambda_opt_params_loop_2e3b
.L_lambda_opt_params_end_2e3b:
	add rsp, rbx

...
module Reader : READER = struct
  ...
  and nt_vector str =
    ...
    (* let nt3 = pack(caten nt1 nt2) (fun _ -> ScmVector[]) in *)
    let nt4 = star nt_sexpr  in
    let nt5 = caten nt4 nt2 in
    let nt5 = pack(caten nt1 nt5) (fun (_,(sexpr,_))->ScmVector sexpr)  in
    (* let nt6 = disj nt3 nt5 in  *)
    nt5 str  
  ...
  (* let nt1 = char '#' in
     ...
     let nt3 = pack(caten nt3 (char ')')) (fun (sexpr,_ )->ScmVector sexpr) in 
...
module Tag_Parser (* : TAG_PARSER *) = struct
  ...
  let rec macro_expand_cond_ribs ribs = 
    match ribs with
    | ScmNil -> ScmVoid
    | ScmPair (ScmPair (ScmSymbol "else", expr), ribs) ->
       ScmPair (ScmSymbol "begin", expr)
    | ScmPair (ScmPair (expr,
                        ScmPair (ScmSymbol "=>",
                                 ScmPair (expr_f, ScmNil))), ribs) ->
       let continue = macro_expand_cond_ribs ribs in
       ScmPair (ScmSymbol "let", ScmPair (ScmPair (ScmPair (ScmSymbol "e", ScmPair (expr, ScmNil)),
                                                   ScmPair (ScmPair ( ScmSymbol "fth",ScmPair (ScmPair (
                                                                                                   ScmSymbol "lambda",
                                                                                                   ScmPair (ScmNil, ScmPair (expr_f, ScmNil))),ScmNil)),
                                                            ScmPair (ScmPair (ScmSymbol "rest", ScmPair (
                                                                                                    ScmPair (
                                                                                                        ScmSymbol "lambda",
                                                                                                        ScmPair (ScmNil,ScmPair (continue, ScmNil))),ScmNil)),
                                                                     ScmNil))),
                                          ScmPair (ScmPair (ScmSymbol "if",ScmPair (ScmSymbol "e",ScmPair (ScmPair (
                                                                                                               ScmPair (ScmSymbol "fth", ScmNil), ScmPair (ScmSymbol "e", ScmNil)),
                                                                                                           ScmPair (ScmPair (ScmSymbol "rest", ScmNil), ScmNil)))),
                                                   ScmNil)))
    | ScmPair (ScmPair (pred, exprs), ribs)->
       let remaining = macro_expand_cond_ribs ribs in
       ScmPair (ScmSymbol "if",ScmPair (pred,ScmPair (ScmPair (ScmSymbol "begin", exprs),ScmPair (remaining, ScmNil))))
    | _ -> raise (X_syntax "failed to cond");;

  
  let rec letrec_dummy_value  = function
    | ScmNil -> ScmNil
    | ScmPair(ScmPair(ScmSymbol var, _), rest) -> 
       ScmPair(ScmPair(ScmSymbol var, ScmPair
                                        (ScmPair (ScmSymbol "quote", ScmPair(ScmSymbol "dummy_value", ScmNil)), ScmNil))
             , letrec_dummy_value rest)
    | _ -> raise (X_syntax "syntax error in letrec");;

  
  let rec set_letrec_vars_values var expr = 
    match var with
    | ScmNil -> expr
    | ScmPair(ScmPair(ScmSymbol var, ScmPair (value, ScmNil)),rest) -> 
       ScmPair(ScmPair(ScmSymbol "set!", ScmPair(ScmSymbol var ,ScmPair (value, ScmNil)))
             , set_letrec_vars_values rest expr)
    | _ ->raise (X_syntax "syntax error in letrec");;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       (*raise (X_not_yet_implemented "hw 1")*)
       let vars = letrec_dummy_value ribs in
       let set_true_values = set_letrec_vars_values ribs exprs in
       tag_parse(ScmPair(ScmSymbol "let", ScmPair(vars , set_true_values)))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address pe =
    let rec modify_AST expr params env =
      match expr with
      | ScmConst const_expr -> ScmConst' const_expr
      | ScmVarGet(Var var) -> ScmVarGet'(tag_lexical_address_for_var var params env)
      | ScmIf(pred, do_if_true, do_if_false) -> 
         ScmIf'(modify_AST pred params env, modify_AST do_if_true params env, modify_AST do_if_false params env)
      | ScmSeq seq_exprs -> ScmSeq' (List.map (fun expr -> modify_AST expr params env) seq_exprs)
      | ScmOr or_exprs -> ScmOr' (List.map (fun or_rib -> modify_AST or_rib params env) or_exprs)
      | ScmVarSet (Var var, val_expr) -> 
         ScmVarSet' (tag_lexical_address_for_var var params env, modify_AST val_expr params env)
      | ScmVarDef (Var var, val_expr) -> 
         ScmVarDef' (tag_lexical_address_for_var var params env, modify_AST val_expr params env)
      | ScmLambda (lambda_params, Simple, lambda_body) -> 
         ScmLambda' (lambda_params, Simple, modify_AST lambda_body lambda_params (params::env))
      | ScmLambda (lambda_params, Opt opt_params, lambda_body) ->
         ScmLambda' (lambda_params, Opt opt_params, modify_AST lambda_body (lambda_params @ [opt_params]) (params::env))
      | ScmApplic (applic_proc,applic_inputs) ->
         ScmApplic'(modify_AST applic_proc params env, 
                    List.map (fun applic_input -> modify_AST applic_input params env) applic_inputs, Non_Tail_Call) in
    
    modify_AST pe [] [];;

  (* run this second *)
  let annotate_tail_calls pe = 
    let rec tag_call is_tail_call = function
      | (ScmConst' _) | (ScmVarGet' _) as orig -> orig
      | ScmIf' (pred, do_if_true, do_if_false) ->
         ScmIf'(tag_call false pred, tag_call is_tail_call do_if_true, tag_call is_tail_call do_if_false)
      | ScmSeq' (exprs) ->
         (match list_and_last exprs with
          | Some (exprs, expr) ->
             let exprs = List.map (fun expr -> tag_call false expr) exprs in
             let expr = tag_call is_tail_call expr in
             ScmSeq'(exprs @ [expr])
      ...
      | ScmVarSet' (tagged_var, tagged_expr) ->
         ScmVarSet'(tagged_var, tag_call false tagged_expr)
      | ScmVarDef' (tagged_var, tagged_expr) ->
         ScmVarDef'(tagged_var, tag_call false tagged_expr)
      | (ScmBox' _) | (ScmBoxGet' _) as expr' -> expr'
      | ScmBoxSet' (tagged_var, tagged_expr) ->
         ScmBoxSet' (tagged_var, tag_call false tagged_expr)
      | ScmLambda' (lambda_params, Simple, lambda_body) ->
         ScmLambda' (lambda_params, Simple, tag_call true lambda_body)
      | ScmLambda' (lambda_params, Opt opt, lambda_body) ->
         ScmLambda' (lambda_params, Opt opt, tag_call true lambda_body)
      | ScmApplic' (applic_proc, applic_inputs, applic_kind) ->
         ScmApplic' (tag_call false applic_proc,
                     List.map (fun applic_input -> tag_call false applic_input) applic_inputs,
                     if is_tail_call then Tail_Call else Non_Tail_Call) in
    tag_call false pe;;
...
module Code_Generation : CODE_GENERATION = struct
  ...
  (* and runs exprs' =
     ...
     global_bindings_table)
     @ (runs pe));; *)
  (*raise (X_not_yet_implemented " project")*);;
  (***************************************************** *)
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmIf' (test, dit, dif) ->
         (*raise (X_not_yet_implemented " project")*)
         let lable_if_else = make_if_else() in
         let label_end = make_if_end() in
         let label_if = (run params env test)
      ...
      | ScmVarSet' (Var' (v, Free), expr') ->
         (*raise (X_not_yet_implemented " project")*)
         let var = (run params env expr') in
         let label = search_free_var_table v free_vars 
         in var
      ...
      | ScmVarSet' (Var' (v, Param minor), ScmBox' _) ->
         "\tmov rdi, 8\n"
         ^ "\tcall malloc\n"  
         ^ (Printf.sprintf "\tmov rdi, qword [rbp + 8 * (4+ %d)]\n" minor)
         ^ "\tmov qword[rax], rdi\n"
      ...
      | ScmBoxSet' (var', expr') ->
         (*raise (X_not_yet_implemented " project")*)
         let exprss = ScmVarGet' var' in
         let var = (run params env expr') in
         var
         ^ "\tpush rax\n"
         ^ (run params env exprss)
         ^ "\tpop qword[rax]\n"
         ^ "\tmov rax, sob_void\n"
      (* ********************************************)
      ...
      | ScmLambda' (params', Opt opt, body) ->
         ...
         ^ (Printf.sprintf "\tje %s ; if we have no opt parameters\n" label_arity_exact) 
         ^ (Printf.sprintf "\tjg %s ; if we have at least one opt parameter\n" label_arity_more) 
         ^ "\tjl L_error_incorrect_arity_opt\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_arity_exact)
         
         ^ (Printf.sprintf "%s:\n" label_arity_exact)
         ^ "\t;;;;goal of this label: lower the params location by 1 , so that we could have room for sob_nil\n\n"
         ^ (Printf.sprintf "\tmov r9, 3 + %d\n" (List.length params'))
         ^ "\tsub rsp, 8\n"                                 
         ...
         ^ (Printf.sprintf "%s:\n" label_opt_loop_exit)
         ^ (Printf.sprintf "\tmov [rsp + 16 + 8 * %d], rax\n" ((List.length params') + 1)) 
         ^ (Printf.sprintf "\tmov r10, %d\n" ((List.length params') + 1)) 
         ^ "\tmov [rsp + 16], r10 ; new params length\n" 
         ^ "\tmov r14, rsp\n"
         ^ (Printf.sprintf "\tadd r14, 16 + 8 * %d ; jump to the top of the parameters\n" ((List.length params') + 1))
